(() => {
  ...
  const initPointerRipples = () => {
    if (reduceMotion || !window.matchMedia("(pointer: fine)").matches) return;

    const canvas = document.createElement("canvas");
    canvas.className = "pointer-ripple-field";
    ...
    const configs = {
      paper: { color: [81, 117, 140], accent: [105, 132, 112], speed: 118, lifetime: 1.85, wavelength: 20, alpha: 0.105, lineWidth: 1.25, rings: 3, corner: 11, moveDistance: 52, moveInterval: 105 },
      research: { color: [102, 145, 158], accent: [128, 163, 169], speed: 92, lifetime: 2.15, wavelength: 23, alpha: 0.12, lineWidth: 1.35, rings: 4, corner: 8, moveDistance: 43, moveInterval: 95 },
      publication: { color: [105, 132, 112], accent: [81, 117, 140], speed: 136, lifetime: 1.22, wavelength: 14, alpha: 0.082, lineWidth: 1, rings: 3, corner: 0, moveDistance: 34, moveInterval: 82 },
      metric: { color: [81, 117, 140], accent: [121, 154, 156], speed: 112, lifetime: 1.18, wavelength: 13, alpha: 0.115, lineWidth: 1.1, rings: 3, corner: 8, moveDistance: 28, moveInterval: 76 }
    };

    let pixelRatio = 1;
    let ripples = [];
    let frameId = null;
    const lastEmission = new WeakMap();
    ...
    const getSurface = (eventTarget) => {
      if (!(eventTarget instanceof Element)) return null;
      const metric = eventTarget.closest(".metric-card");
      if (metric) return { element: metric, kind: "metric" };
      const research = eventTarget.closest("[data-research-visual]");
      if (research) return { element: research, kind: "research" };
      const publication = eventTarget.closest(".publication");
      if (publication) return { element: publication, kind: "publication" };
      const paper = eventTarget.closest("[data-identity-paper]");
      if (!paper || eventTarget.closest("[data-particle-title], [data-hamster-pet]")) return null;
      return { element: paper, kind: "paper" };
    };

    const drawRipple = (ripple, now) => {
      if (!ripple.element.isConnected) return false;
      const age = (now - ripple.startedAt) / 1000;
      if (age >= ripple.config.lifetime) return false;
      const rect = ripple.element.getBoundingClientRect();
      if (rect.bottom < -20 || rect.top > window.innerHeight + 20) return false;

      const x = rect.left + ripple.localX;
      const y = rect.top + ripple.localY;
      const progress = age / ripple.config.lifetime;
      const temporalDecay = Math.exp(-1.15 * age) * Math.pow(1 - progress, 0.72);
      const front = 5 + ripple.config.speed * age;

      context.save();
      roundedRect(rect.left, rect.top, rect.width, rect.height, ripple.config.corner);
      if (ripple.kind === "paper") {
        ripple.element.querySelectorAll("[data-pet-avoid]").forEach((element) => {
          const protectedRect = element.getBoundingClientRect();
          context.rect(protectedRect.left - 3, protectedRect.top - 3, protectedRect.width + 6, protectedRect.height + 6);
        });
        context.clip("evenodd");
      } else {
        context.clip();
      }
      for (let index = 0; index < ripple.config.rings; index += 1) {
        const radius = front - index * ripple.config.wavelength;
        if (radius <= 1) continue;
        const spatialDecay = Math.exp(-index * 0.54) * Math.exp(-radius * 0.0018);
        const alpha = ripple.config.alpha * ripple.strength * temporalDecay * spatialDecay;
        const color = index % 2 === 0 ? ripple.config.color : ripple.config.accent;
        context.beginPath();
        context.arc(x, y, radius, 0, Math.PI * 2);
        context.strokeStyle = `rgba(${color[0]}, ${color[1]}, ${color[2]}, ${alpha.toFixed(4)})`;
        context.lineWidth = Math.max(0.55, ripple.config.lineWidth * (1 - progress * 0.38));
        context.stroke();
      }
      context.restore();
      return true;
    ...
    const render = (now) => {
      ...
      if (ripples.length && !document.hidden) frameId = window.requestAnimationFrame(render);
    };
    ...
    const emit = (surface, event, strength, now = performance.now()) => {
      const rect = surface.element.getBoundingClientRect();
      if (!rect.width || !rect.height) return;
      ripples.push({
        element: surface.element,
        kind: surface.kind,
        config: configs[surface.kind],
        localX: Math.max(0, Math.min(rect.width, event.clientX - rect.left)),
        localY: Math.max(0, Math.min(rect.height, event.clientY - rect.top)),
        strength,
        startedAt: now
      });
    ...
    const handleMove = (event) => {
      ...
      if (!surface) return;
      const now = performance.now();
      const config = configs[surface.kind];
      const previous = lastEmission.get(surface.element);
      if (!previous) {
        lastEmission.set(surface.element, { x: event.clientX, y: event.clientY, time: now });
        emit(surface, event, 0.72, now);
        return;
      }
      const distance = Math.hypot(event.clientX - previous.x, event.clientY - previous.y);
      const elapsed = now - previous.time;
      if (distance < config.moveDistance || elapsed < config.moveInterval) return;
      const velocity = distance / Math.max(elapsed, 1);
      const strength = Math.min(1, 0.62 + velocity * 0.28);
      lastEmission.set(surface.element, { x: event.clientX, y: event.clientY, time: now });
      emit(surface, event, strength, now);
    };

    const handlePress = (event) => {
      if (event.pointerType === "touch") return;
      const surface = getSurface(event.target);
      if (surface) emit(surface, event, 1.32);
    };
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.hidden && frameId) {
        window.cancelAnimationFrame(frameId);
        frameId = null;
      } else if (!document.hidden && ripples.length) {
        ensureAnimation();
      }
    });
  };
